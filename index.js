...
const ownerNumber = "5583986694715"
const ownerName = "Araújo"

//_LISTA DE CRS
const cr = "Ademir domina🐊🚩"
const crfig = "Fig criada com sucesso🔧"
...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Araújo‍️\n' 
            + 'ORG:Dono do Tiringa;\n'
            + 'TEL;type=CELL;type=VOICE;waid=5583986694715:+55 83 8669-4715\n' 
            + 'END:VCARD'
    ...
    / tiringa.on('CB:Blocklist', json => {
      ...
      tiringa.on('chat-update', async (mek) => {
        ...
        '🍋 : 🍋 : 🍋',
			...
			const ownerNumber = [`5583986694715@s.whatsapp.net`]
...
//_FUCTION PATENTE
        ...
        if (levelRole === 1) {
            ...
            role = 'Recruta Experiente'
        } else if (levelRole === 3) {
            role = 'Recruta Moral'
        } else if (levelRole === 4) {
            role = 'Soldado'
        } else if (levelRole === 5) {
            role = 'Soldado Experiente'
        } else if (levelRole === 6) {
            role = 'Cabo'
        } else if (levelRole === 7) {
            role = 'Cabo Experimente'
        } else if (levelRole === 8) {
            role = 'Cabo Pre promovido'
        } else if (levelRole === 9) {
            role = 'Sargento Experiente'
...
`  ┏━━━🥇 PATENTE 🥇━━━┓
  ...
  ┃│‣ Patente: ${role}
  ┃│
  ┃│‣ Suba de patente sendo um bom soldado! 
  ┃│ converse no grupo para ganhar experiência.
  ┃╰────────────
  ┗━━━━━━━━━━━━━┛`
...
//_FUCTION NOME DOS STICKERS
				...
				if (!packname) packname = '@Rei_dus_shit'; if (!author) author = 'Bot';	
...
｡☆✼━━━━━━ ★ ━━━━━━✼☆｡
      ⚠️AVISO:
•*Alguns comandos estão com limite de uso. Então não use exageradamente*
...
┏━━━━━━ ★ ━━━━━━༺
┠⊱ ${prefix}st 
┃ _Cria uma figurinha em 512x512_
┠⊱ ${prefix}fig
┃ _Transforma imagem em fig_
┠⊱ ${prefix}brabo
┃ _Cria uma fig no estilo triggered_
┠⊱ ${prefix}wasted
┃ _Cria uma fig no estilo wasted_
┠⊱ ${prefix}toimg
┃ _inverte fig em imagem_
┗━━━━━━ ★ ━━━━━━༺
...
┃ _Abre o grupo_
┠⊱ ${prefix}promover
┃ _Promove membro a administrador_
┠⊱ ${prefix}rebaixar
┃ _Rabaixa o adm a membro comum_
┠⊱ ${prefix}gnome
...
case 'pão':
if (isOwner) return reply('Oi Araújo')
break 
...
yhb = {
text: `Que fofo... @${nomor.split("@s.whatsapp.net")[0]} deu um abraço gsotoso do urso em @${mentioned[0].split('@')[0]}`,
...
tiringa.sendMessage(from, italuft, image, {quoted: mek, caption: `☆━✪🕴  ARAÚJO  🕴✪━☆\n\nChat do baiano: wa.me/+5583986694715`})
break
...
case 'info':
 ...
 `‣ Nome do bot: ${me.name}
  ...
  ‣ Dono: 'https://instagram.com/euherbson.ofc?igshid=3jkc2cbldq3o'
  ‣ Prefix: ${prefix}
...
case 'liberado:
					...
					//if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply(`Digite ${prefix}liberado 1 para ativar`)
					if (Number(args[0]) === 1) {
						if (isNsfw) return reply('O +18 já está ativo no grupo')
						nsfw.push(from)
						fs.writeFileSync('./data/nsfw.json', JSON.stringify(nsfw))
						reply('+18 ja ta ativado no grupo')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./data/nsfw.json', JSON.stringify(nsfw))
						reply('+18 desativado no grupo')
					} else {
						reply(`Digite ${prefix}liberado 1 para ativar, 0 para desativar o recurso`)
					}
					break
...
//_VOZ DO GOOGLE
				case 'tts':
					...
					ranm = getRandom('.mp3')
					dtt.length > 800
					? reply('Texto muito grande, máximo 800 caracteres')
...
mentions('╭╾╼◐⚋ ༒ ᴛᴏᴅᴏs ༒⚋◑╾╼╮\n┃➸'+todos+'╰╾╼◐⚋⚋ ༒TIRINGA-BOT༒ ⚋⚋◑╾╼╯', members_id, true)
break

//_LIMPA TODOS OS CHATS
				case 'clearall':
				case 'limparchat':
					if (!isOwner) return reply('Só o Araujo pode fazer isso')
...
case 'chat off':
  ...
  var nomor = mek.participant
  const close = {
text: `Grupo fechado! somente admins podem falar.
contextInfo: {
  mentionedJid: [nomor]
}
  }
...
if (!isBotGroupAdmins) return reply(mess.only.Badmin)
open = {
  text: `Pronto! agora todos podem falar.
  contextInfo: {
mentionedJid: [sender]
  }
}
...
case 'bv':
				    ...
				    if (isWelkom) return reply('já está ativado')
						...
						reply(`digite ${prefix}bv 1 para ativar, e 0 para desativar o recurso`)
					}
                         break
